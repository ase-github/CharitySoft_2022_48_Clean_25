...
Public Class UCBeneficiarPeriodicalReportsStatus
...
#Region "Events"
    ...
    Protected Sub ASPxCallback_Save_Callback(source As Object, e As DevExpress.Web.CallbackEventArgs) Handles ASPxCallback_Save.Callback
        e.Result = ApproveReports()
    End Sub

#End Region

    Private Function ApproveReports() As String

        Dim Reports = ASPxGridView1.GetSelectedFieldValues("ReportID")
        If Reports.Count = 0 Then
            Return "الرجاء اختيار تقرير واحد على الاقل"
        Else
            str = ""
            For Each report In Reports
                str = str & " UPDATE TBL_BeneficiarPeriodicalReports SET ReportStatus=1 , ApprovedDate='" & Date.Now.ToString("MM/dd/yyyy HH:mm:ss") & "' , AprovedBy='" & Session(SessionKey.USERNAME) & "' WHERE ReportID=" & report & "; "
            Next
