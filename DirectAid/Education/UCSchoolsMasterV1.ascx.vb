Public Class UCSchoolsMasterV1
...
#Region "Private Methods"
    Sub CreateNew()
        ...
        SchoolManagerRemarks.Value = Nothing
        FieldOfficeManagerRemarks.Value = Nothing
        GeoSectorRemarks.Value = Nothing
        EducationSectionRemarks.Value = Nothing
        governrate_id.Value = Nothing
        Vacation.Value = Nothing
    ...
    Sub SaveNew(ByRef db As Database)
        ...
        Dim val_terms_num As Byte = 0

        If Byte.TryParse(ddl_terms_num.Value, val_terms_num) Then
            school_new.terms_num = val_terms_num
        Else
            school_new.terms_num = Nothing
        End If
        ...
        school_new.SchoolManagerRemarks = SchoolManagerRemarks.Value
        school_new.FieldOfficeManagerRemarks = FieldOfficeManagerRemarks.Value
        school_new.GeoSectorRemarks = GeoSectorRemarks.Value
        school_new.EducationSectionRemarks = EducationSectionRemarks.Value
        school_new.governrate_id = governrate_id.Value
        school_new.Vacation = Vacation.Value
        school_new.nat_code = ddl_country_name.Value

        school_new.school_related_to_da = 1
        db.school_master.AddObject(school_new)
        db.SaveChanges()
        DevExpress.Web.ASPxWebControl.RedirectOnCallback("SchoolsMaster.aspx?scode=" + school_new.school_code.ToString())
    End Sub
    ...
    Sub UpdateUIWithData(ByRef db As Database)
        Dim ID = Integer.Parse(txt_hdn_scode.Text)
        Dim school_new As school_master = db.school_master.Where(Function(o) o.school_code = ID And o.school_related_to_da = 1).FirstOrDefault()
        If Not school_new Is Nothing Then
            ...
            '    governrate_id.Value = school_new.governrate_id
            'End If

            ddl_country_name.Value = school_new.nat_code
            ddl_country_city.Value = school_new.country_city
            governrate_id.Value = school_new.governrate_id
            ...
            Dim value_terms_num As Int16? = 0

            If Int16.TryParse(school_new.terms_num, value_terms_num) Then

                ddl_terms_num.Value = value_terms_num

            Else

                ddl_terms_num.Value = Nothing

            End If

            SchoolManagerRemarks.Value = school_new.SchoolManagerRemarks
            FieldOfficeManagerRemarks.Value = school_new.FieldOfficeManagerRemarks
            GeoSectorRemarks.Value = school_new.GeoSectorRemarks
            EducationSectionRemarks.Value = school_new.EducationSectionRemarks
            Vacation.Value = school_new.Vacation
    ...
    Function ValidateOnSave() As String
        ...
        If ddl_terms_num.Value Is Nothing Then
            message += "برجاء ادخال النظام الدراسي المتبع." + Environment.NewLine
        End If
...
#End Region
    ...
    Protected Sub ASPxButton_New_Click(sender As Object, e As EventArgs) Handles ASPxButton_New.Click
        Response.Redirect("SchoolsMaster.aspx")
    End Sub
